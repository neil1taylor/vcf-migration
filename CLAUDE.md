...
- **Export Pipeline**: `src/services/export/` — `bomXlsxGenerator.ts` (ExcelJS), `pdfGenerator.ts` (jsPDF), `excelGenerator.ts`, `docxGenerator.ts`, `yamlGenerator.ts`, `handoverExporter.ts` (bundles RVTools file + localStorage settings into a single download for colleague handoff). **Import**: `src/services/settingsExtractor.ts` extracts settings from a handover file without full parsing; `src/services/settingsRestore.ts` writes them to localStorage. Available on Settings and Export pages.
