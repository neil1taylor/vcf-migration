...
export function generateVSIRemediationItems(counts: PreflightCheckCounts): RemediationItem[] {
  ...
  if (counts.vmsWithUnsupportedOS && counts.vmsWithUnsupportedOS > 0) {
    items.push({
      ...
      severity: 'blocker',
      description: 'These VMs have operating systems without IBM stock images. IBM provides validated stock images for Windows Server 2012+, RHEL 8+, Ubuntu 18.04+, CentOS 8+, Debian 10+, SLES, Rocky Linux, and AlmaLinux. Most 64-bit x86-64 operating systems can run on VPC VSIs, but custom images require customer validation.',
...
export function generateVSIAllChecks(counts: PreflightCheckCounts): RemediationItem[] {
  ...
  items.push({
    ...
    severity: 'unknown',
    description: 'VPC VSI uses virtio drivers for storage and networking. Linux VMs typically include these drivers. Windows VMs require RedHat virtio drivers to be installed or injected during migration. Driver availability varies by Windows version: current virtio-win (0.1.215+) supports Windows Server 2012 R2+, Windows Server 2008 R2 requires archived drivers (0.1.173.9), and Windows Server 2003 has no viable drivers. This cannot be verified from RVTools data.',
    remediation: 'For Linux: Verify virtio drivers are loaded with "lsmod | grep virtio". Most modern distributions (RHEL 7+, Ubuntu 18.04+) include these by default. For Windows Server 2012 R2+: Use current RedHat virtio-win drivers from https://fedorapeople.org/groups/virt/virtio-win/. For Windows Server 2008 R2: Archived virtio-win-0.1.173.9 from https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/archive-virtio/ may work but is unsupported — OS upgrade to 2019/2022 strongly recommended. For Windows Server 2003: No viable VirtIO drivers exist — OS upgrade or re-platforming required. Migration tools like virt-v2v and RackWare can inject drivers automatically for supported OS versions.',
    documentationLink: VPC_DOCS.fullvalenceMigration,
    affectedCount: 0,
    affectedVMs: [],
    isUnverifiable: true,
